...
class DoubleHashing {
public:
    explicit DoubleHashing(int size = 101, int r = 89) : array_(NextPrime(size)), current_size_(0), collisions_(0), r_value_(r) { }
...
private:
    ...
    int FindPos(const HashedObj &x, int &probes) const {
        ...
        int step_size = r_value_ - (current_pos % r_value_);
